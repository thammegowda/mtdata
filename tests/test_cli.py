...
def shrun(cmd: List[str], capture_output=False):
...
def test_cli_help():
    assert shrun(MTDATA_CMD + ['--help']) == 0

def test_cli_list():
    code, out = shrun(MTDATA_CMD + ['list', '--id'], capture_output=True)
    assert code == 0
...
def test_cli_get():
    with TemporaryDirectory() as out_dir:
        ...
        assert shrun(MTDATA_CMD + ['get', '-l', 'eng-kan', '-tr', did, '-o', out_dir]) == 0
        assert (Path(out_dir) / 'mtdata.signature.txt').exists()

def test_cache():
    code = shrun(MTDATA_CMD + ['cache', '-ri', 'tg01_2to1_test', '-j3'], capture_output=False)
    assert code == 0


def test_get_recipe():
    with TemporaryDirectory() as out_dir:
        code = shrun(MTDATA_CMD + ['get-recipe', '-ri', 'tg01_2to1_test', '-o', out_dir], capture_output=False)
        assert code == 0
...
def test_metadata():
    with TemporaryDirectory() as out_dir:
        ...
        code = shrun(MTDATA_CMD + ['get', '-l', 'slv-eng', '-tr', did, '-o', str(out_dir)], capture_output=False)
        assert code == 0
